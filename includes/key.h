/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   key.h                                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: fsaffiri <fsaffiri@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/23 12:56:44 by fosuna-g          #+#    #+#             */
/*   Updated: 2025/11/17 11:42:19 by fsaffiri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef KEY_H
# define KEY_H

# define EVENT_CLOSE_BTN 17 // To close the window and finish the program
# define KEY_ESC 65307 // To close the window and finish the program
# define KEY_W 119 // To move foward
# define KEY_A 97 // To move to the left
# define KEY_S 115 // To move backward
# define KEY_D 100 // To move to the right
# define KEY_UP 65362 // Upward arrow
# define KEY_DOWN 65364 // Downward arrow
# define KEY_RIGHT 65363 // Right arrow
# define KEY_LEFT 65361 // Left arrow
# define KEY_E 101 // E key
# define KEY_C 99 // C key
# define KEY_N 110 // N key
# define KEY_M 109 // M key
# define KEY_SHIFT 65505 // Shift key
# define KEY_ENTER 65293 // Enter / Return
# define MOUSE_UP 4 // Scrol UP
# define MOUSE_DOWN 5 // Scrol DOWN
# define MOUSE_WHEEL_BTN 3 // Wheel button
# define MOUSE_LEFT 1 // Left click
# define MOUSE_RIGHT 2 // Right click

#endif